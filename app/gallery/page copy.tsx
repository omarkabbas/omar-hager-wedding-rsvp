...
export default function GalleryPage() {
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    setStatus("Uploading...");
    setProgress(15); // Instant jump so it doesn't look "late"
    
    // Faster increment for a more responsive feel
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Hide the bar quickly (1.5 seconds) after success
        setTimeout(() => {
          setProgress(0);
        }, 1500);
      ...
      setUploading(false);
      // Keep success text a bit longer, but the bar will be gone
  ...
  return (
    <div className="min-h-screen bg-[#D0E0F0] text-stone-800 flex flex-col items-center font-sans relative pb-20 overflow-x-hidden">
      ...
      {/* GALLERY SECTION */}
      <section className="max-w-[1200px] w-full mt-16 md:mt-24 px-4 md:px-0">
        ...
        {photos.length === 0 ? (
          ...
          <div className="space-y-10 md:space-y-16">
            {photoRows.map((row, rowIndex) => (
              <div 
                key={`row-${rowIndex}`} 
                className="flex flex-nowrap md:grid md:grid-cols-6 gap-4 md:gap-8 overflow-x-auto snap-x snap-mandatory pb-6 scrollbar-hide w-full"
              ...
              >
                ...
                {row.map((photo: any) => (
                  <div key={photo.id} className="shrink-0 w-[31%] md:w-auto snap-start">
                    <div className="bg-white p-3 md:p-4 shadow-[0_10px_30px_rgba(0,0,0,0.12)] rounded-sm border-[1.5px] border-white active:scale-95 transition-all duration-200">
                      <a 
